<h1 align="center">🦀 clawsh-proto</h1>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/used%20by-clawsh-blue" alt="clawsh">
  <img src="https://img.shields.io/badge/used%20by-clawsh--imp-blue" alt="clawsh-imp">
</p>

Shared wire protocol and cryptography library for the clawsh C2 framework. Used by both the handler (clawsh) and the implant (clawsh-imp).

Supports both Linux and Windows agents over TCP, TLS, and HTTP tunnel transports.
...
| Field | Source (Linux) | Source (Windows) |
|---|---|---|
| `version` | Build-time implant version | same |
| `hostname` | `/proc/sys/kernel/hostname` | `GetComputerNameW` |
| `os_release` | `/etc/os-release` | `RtlGetVersion` + Registry |
| `kernel` | `uname()` release string | `RtlGetVersion` build number |
| `arch` | `uname()` machine string | `GetNativeSystemInfo` |
| `uid` / `gid` | `getuid()` / `getgid()` | Token RID-based pseudo-UIDs |
| `username` | `/etc/passwd` lookup | Token user SID lookup |
| `pid` | `getpid()` | `GetCurrentProcessId` |
| `process_name` | `/proc/self/comm` (post-disguise) | `GetModuleFileNameW` |
| `exec_capabilities` | `memfd_create`, `/dev/shm` exec, `/tmp` exec | temp dir exec, PowerShell access |
| `ebpf_detected` | eBPF monitor or active kprobes | EDR drivers via `K32EnumDeviceDrivers` |
| `seccomp_mode` | `/proc/self/status` → Disabled / Strict / Filter | Always 0 on Windows |
| `os_type` | `"linux"` | `"windows"` (default: `"linux"` for backward compat) |
...
|---|---|---|
| `Builtin { cmd, args }` | Silent | Built-in syscall/NT-API commands (ls, cat, pwd, ...) |
| `ShellExec { command }` | Quiet | Execute shell command |
| `RunModule { name }` | Varies | Run a named recon module |
| `ExecMem { elf_data, args }` | Varies | Execute ELF in memory (Linux) |
| `InteractiveShell` | Loud | Allocate PTY/ConPTY, enter interactive shell |
| `Persist(PersistRequest)` | Varies | Install/remove/list persistence |
| `StartTunnelMode` | Loud | Enter tunnel relay mode |
| `ReconAll` | Silent | Queue all recon modules for scatter execution |
...
`PersistMethod` variants inside `Persist(PersistRequest)`:

| Variant | Platform | Noise | Description |
|---|---|---|---|
| `Cron` | Linux | Quiet | `/var/spool/cron/crontabs/{user}` — `@reboot` + interval |
| `Systemd` | Linux | Quiet | `.service` unit + `wants/` symlink |
| `Bashrc` | Linux | Quiet | Marker-delimited block in `~/.bashrc` |
| `SshKey { pubkey }` | Linux | Silent | Public key into `~/.ssh/authorized_keys` |
| `RegistryRun` | Windows | Quiet | `HKCU\...\Run` registry key |
| `ScheduledTask` | Windows | Loud | Scheduled task via `schtasks.exe` |
| `StartupFolder` | Windows | Quiet | Shortcut in Startup folder |
| `WindowsService` | Windows | Quiet | Direct registry write to `HKLM\Services` |
| `Clean` | Both | Quiet | Remove all installed persistence |
| `List` | Both | Silent | List installed persistence records |

### Interactive Shell

| Type | ID | Description |
|---|---|---|
| `ShellData` | `0x50` | Bidirectional PTY/ConPTY byte stream. |
...
```toml
[dependencies]
clawsh-proto = { git = "https://github.com/y0uall/clawsh-proto", tag = "v2.0.0" }
```
...
### v2.0.0
- Windows support: `os_type` field in `AgentInfo`, 4 Windows persistence methods
- `ReconAll` task variant for scatter recon execution
- HTTP tunnel transport (same wire format, HTTP POST encapsulation)
- Beacon interval default changed to 5s

### v0.1.0
- Initial release: wire format, X25519/ChaCha20 crypto, 25 message types
