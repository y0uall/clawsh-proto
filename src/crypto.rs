...
impl KeyPair {
    ...
    /// ECDH + HKDF into a session key. Consumes the key pair.
    ...
    /// Like `derive_session_key`, but returns split enc/dec keys for concurrent I/O.
...
/// ChaCha20-Poly1305 session key — encrypts at the message layer, independent of TLS.
...
/// Encryption key for the write half of a connection. Owns the nonce counter.
...
/// Decryption key for the read half. Stateless — nonce is embedded in each ciphertext.
